...
def text_stat(df, txt_col):
    for col in tqdm(txt_col, desc="Processing text columns"):
        ...
        df[f'{col}_sentence_length_max'] = df[col].apply(
            lambda x: max([len(sentence.split()) for sentence in x.split('.') if sentence.strip()], default=0))
        df[f'{col}_sentence_length_min'] = df[col].apply(
            lambda x: min([len(sentence.split()) for sentence in x.split('.') if sentence.strip()], default=0))
...
base = AbdBase(train_data=train, test_data=test, target_column='winner',gpu=False,
                 ...
                 fold_type='SKF',weights=None,tf_vec=False)
